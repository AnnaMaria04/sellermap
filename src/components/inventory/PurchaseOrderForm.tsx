...
export function PurchaseOrderForm({ onClose, onSave }: Props) {
  ...
  function updateLine(productId: string, field: "qty" | "unitCost", value: number) {
    setLines((prev) =>
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto flex h-full w-full max-w-2xl flex-col bg-[var(--c-bg)] shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-[var(--c-border)] px-6 py-4">
          <div>
            <h2 className="text-lg font-semibold text-[var(--c-text)]">Новый заказ поставщику</h2>
            <p className="text-xs text-[var(--c-text2)]">Создайте заказ для пополнения запасов</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-6">
              ...
              {supplier && (
                <div className="rounded-xl border border-[var(--c-border)] bg-[var(--c-bg3)] p-4">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <InfoRow label="Контакт" value={supplier.contactName ?? "—"} />
                    <InfoRow label="Телефон" value={supplier.phone ?? "—"} />
                    <InfoRow label="Мин. заказ" value={supplier.minOrderQty ? `${supplier.minOrderQty} шт` : "—"} />
                    <InfoRow label="Срок доставки" value={`${supplier.leadTimeDays} дней`} />
                    <InfoRow label="Валюта" value={supplier.currency} />
                    <InfoRow label="Условия оплаты" value={supplier.paymentTerms ?? "—"} />
                  </div>
            ...
            <Section title="Товары">
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 z-50 rounded-xl border border-[var(--c-border)] bg-[var(--c-bg2)] shadow-xl">
                    ...
                    <div className="max-h-60 overflow-y-auto p-1">
                      {filteredProducts.map((p) => {
                        const alreadyAdded = lines.find((l) => l.productId === p.id);
                        return (
                          <button
                            key={p.id}
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-[var(--c-text3)]">{p.sku} · {p.costPrice.toLocaleString("ru-RU")} ₽</p>
                            </div>
                            {alreadyAdded && <Check size={14} className="text-[var(--c-green)] shrink-0" />}
                          </button>
                        );
                      })}
                      {filteredProducts.length === 0 && (
                        <p className="py-4 text-center text-sm text-[var(--c-text3)]">Товары не найдены</p>
                      )}
                    </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-[var(--c-border)] bg-[var(--c-bg3)]">
                        <th className="px-4 py-2 text-left text-xs font-medium text-[var(--c-text2)]">Товар</th>
                        <th className="px-4 py-2 text-right text-xs font-medium text-[var(--c-text2)]">Кол-во</th>
                        <th className="px-4 py-2 text-right text-xs font-medium text-[var(--c-text2)]">Цена ед.</th>
                        <th className="px-4 py-2 text-right text-xs font-medium text-[var(--c-text2)]">Сумма</th>
                        <th className="w-8 px-4 py-2" />
                      </tr>
                    </thead>
                    <tbody>
                      {lines.map((line) => (
                        <tr key={line.productId} className="border-b border-[var(--c-border)] last:border-0">
                          ...
                          <td className="px-4 py-3">
                            <input
                              ...
                              min={1}
                              value={line.qty}
                              onChange={(e) => updateLine(line.productId, "qty", parseInt(e.target.value) || 1)}
                              className="w-20 rounded-lg border border-[var(--c-border2)] bg-[var(--c-bg3)] px-2 py-1.5 text-right text-sm text-[var(--c-text)] focus:border-[var(--c-green)] focus:outline-none tabular"
                            />
                          </td>
                          <td className="px-4 py-3">
                            <div className="relative w-28">
                              <span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-[var(--c-text3)]">₽</span>
                              <input
                                type="number"
                                min={0}
                                value={line.unitCost}
                                onChange={(e) => updateLine(line.productId, "unitCost", parseFloat(e.target.value) || 0)}
                                className="w-full rounded-lg border border-[var(--c-border2)] bg-[var(--c-bg3)] pl-5 pr-2 py-1.5 text-right text-sm text-[var(--c-text)] focus:border-[var(--c-green)] focus:outline-none tabular"
                              />
                            </div>
                          </td>
                          <td className="px-4 py-3 text-right tabular">
                            <span className="text-sm font-medium text-[var(--c-text)]">
                              {(line.qty * line.unitCost).toLocaleString("ru-RU")} ₽
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <button
                    ...
                    <tfoot>
                      <tr className="border-t border-[var(--c-border)] bg-[var(--c-bg3)]">
                        <td colSpan={3} className="px-4 py-3 text-right text-sm font-medium text-[var(--c-text2)]">
                          Итого:
                        </td>
                        <td className="px-4 py-3 text-right tabular">
              ...
              {supplier?.minOrderQty && lines.some((l) => l.qty < supplier.minOrderQty!) && (
                <div className="flex items-start gap-2 rounded-lg bg-[var(--c-amber-dim)] border border-[rgba(245,166,35,0.2)] p-3">
                  <AlertCircle size={15} className="shrink-0 mt-0.5 text-[var(--c-amber)]" />
                  <p className="text-xs text-[var(--c-amber)]">
                    Минимальный заказ у поставщика: {supplier.minOrderQty} шт. Проверьте количество.
                  </p>
                </div>
              )}
            </Section>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={cn(
                ...
                isValid && !saved ? "text-[var(--c-text2)] hover:text-[var(--c-text)]" : "opacity-40 cursor-not-allowed text-[var(--c-text3)]",
              )}
            >
              <FileText size={14} />
              Черновик
            ...
            >
              {saved ? <><Check size={14} /> Сохранено</> : "Отправить поставщику"}
            </button>
          </div>
        </div>
      </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="mb-4 border-b border-[var(--c-border)] pb-2 text-sm font-semibold text-[var(--c-text)]">{title}</h3>
...
const inputCls = "h-9 w-full rounded-lg border border-[var(--c-border2)] bg-[var(--c-bg3)] px-3 text-sm text-[var(--c-text)] placeholder:text-[var(--c-text3)] focus:border-[var(--c-green)] focus:outline-none transition";
const selectCls = "h-9 w-full rounded-lg border border-[var(--c-border2)] bg-[var(--c-bg3)] px-3 text-sm text-[var(--c-text)] focus:border-[var(--c-green)] focus:outline-none transition";
