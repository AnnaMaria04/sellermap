...
These remain open because they require either the **front-end application code
that is not present in this repository** (this repo currently contains only the
Marketplace Intelligence app and the inventory *foundation* library — not the
`inventory-store`, hooks, or panels the audit references) or because there is
**no source data to backfill**:
